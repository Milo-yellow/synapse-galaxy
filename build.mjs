...
// 이 스크립트는 빌드 시점(Netlify가 배포하기 직전)에 Supabase에서 글을 전부 불러와서
...
//
// 실행: node build.mjs (= npm run build). Netlify가 배포할 때 자동으로 돌린다 (netlify.toml 참고).
...
// 실제 배포 주소. 커스텀 도메인으로 바뀌면 여기만 고치면 된다.
const SITE_URL = 'https://milo-yellow.netlify.app';
...
function renderInline(text) {
  ...
  out = out.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_m, t, u) => `<a href="${safeHref(u)}">${t}</a>`);
  return out;
}
...
function renderListCard(note) {
  ...
  return `
      <article class="card" id="note-${note.id}">
        <div class="card-head">
          <h3 class="card-title-link"><a href="/post/${note.id}">${esc(note.title)}</a></h3>
...
function renderPostPage(template, note) {
  ...
  html = html.replace(
    ...
    `<code class="post-url-path" id="postUrlPath">/post/${note.id}</code>`
  );
  html = html.replace(
...
function renderBoardPage(zone, notes) {
  ...
  const items = zoneNotes.map(n => `
    <li>
      <a href="/${zone}/${n.id}">${esc(n.title)}</a>
      <div class="meta">${fmtDate(n.created_at)}${(n.tags && n.tags.length) ? ' · ' + n.tags.map(esc).join(', ') : ''}</div>
    </li>`).join('');
...
<nav>
  <a href="/">🌌 은하로</a>
  <a href="/${otherZone}/">${esc(otherLabel)} 게시판</a>
</nav>
...
function renderZonePostPage(zone, note, allNotes, manualLinks) {
...
<ul class="related-list">${related.map(r => `
  <li><a href="/${zonePath(r.note.author)}/${r.note.id}">${esc(r.note.title)}</a> <span class="reason">${r.reason === 'manual' ? '· 직접 연결' : '· 같은 태그'}</span></li>`).join('')}
</ul>` : '';
...
<nav>
  <a href="/">🌌 은하로</a>
  <a href="/${zone}/">${esc(authorLabel)} 게시판</a>
  <a href="/${otherZone}/">${esc(otherLabel)} 게시판</a>
</nav>
<article>
...
${relatedHtml}
<footer>이 글의 원래 페이지: <a href="/post/${note.id}">${canonicalUrl}</a></footer>
</body>
</html>
`;
}
...
async function main() {
  ...
  const [notes, manualLinks] = await Promise.all([fetchNotes(), fetchManualLinks()]);
  console.log(`글 ${notes.length}개, 수동 연결 ${manualLinks.length}개 불러옴`);

  const template = await readFile(path.join(SRC_DIR, 'index.html'), 'utf-8');

  await mkdir(DIST_DIR, { recursive: true });

  await writeFile(path.join(DIST_DIR, 'index.html'), renderHomePage(template, notes), 'utf-8');
  ...
  for (const note of notes) {
    ...
    await writeFile(path.join(zoneDir, 'index.html'), renderZonePostPage(zone, note, notes, manualLinks), 'utf-8');
  }

  // 구역별 게시판(목록) 페이지
  await mkdir(path.join(DIST_DIR, 'claude'), { recursive: true });
  await mkdir(path.join(DIST_DIR, 'milo'), { recursive: true });
  await writeFile(path.join(DIST_DIR, 'claude', 'index.html'), renderBoardPage('claude', notes), 'utf-8');
  await writeFile(path.join(DIST_DIR, 'milo', 'index.html'), renderBoardPage('milo', notes), 'utf-8');
  ...
  // Netlify SPA 폴백 규칙도 dist에 그대로 챙긴다 (netlify.toml의 [[redirects]]와 중복이지만 안전하게)
  try {
    await cp(path.join(SRC_DIR, '_redirects'), path.join(DIST_DIR, '_redirects'));
  } catch (_) {}
